...
const Projects = () => {
  ...
  useEffect(() => {
    if (searchQuery === "all") {
      ...
      const filteredProjects = getProjects?.filter(
        (item) =>
          item?.projectCategory.toLowerCase() === searchQuery.toLowerCase()
      );
    ...
    >
      <div className="col-parent-container common-container">
        ...
        {/* <div className="filter-discription-container">
          <div className="filter-discription-button-container">
            <p>{`${filterProjects?.length} projects`}</p>
          </div> */}
        ...
        {/* <div className="filter-discription-button-container">
            <button onClick={handleProjectsViews}>
              <GrProjects />
            </button>
            <button onClick={handleDescriptionViews}>
              <GoProjectSymlink />
            </button>
            {searchQuery}
        //   </div> */}
        ...
        {descriptionView && (
          <div className="projects-description-view-container">
            {filterProjects?.length === 0 ? (
              <p style={{ color: "#e74c3c" }}>No Projects Found</p>
            ) : (
              filterProjects?.map((item) => (
                <div
                  className="project-description-child-container"
                  key={item?.id}
                >
