    ...
    db.run(
      ...
      function (err) {
        if (err) {
          ...
          return res
            .status(500)
            .json({ status: 500, message: "Failed to add your details" });
        }
...
router.get("/my-details", (req, res) => {
  db.all("SELECT * FROM myDetails", [], (err, rows) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ status: 500, message: "Failed to fetch your details" });
    }
  ...
  (req, res) => {
    ...
    db.get("SELECT resume FROM myDetails WHERE id = ?", [id], (err, row) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ status: 500, message: "Failed to fetch existing data" });
      }
...
router.delete("/my-details/:id", authMiddleware, (req, res) => {
  ...
  db.run("DELETE FROM myDetails WHERE id = ?", [id], function (err) {
    if (err) {
      console.error(err);
      return res
        .status(500)
        .json({ status: 500, message: "Failed to delete your details" });
    }
